...
def get_data_discapacidades1(file_path="../db/discapacidades1.xlsx"):
    ...
    suma_columna_3 = sum(
        row[3].value for row in dataframe.iter_rows(min_row=2, max_row=dataframe.max_row) 
    ...
    for row in dataframe.iter_rows(min_row=2, max_row=dataframe.max_row, max_col=dataframe.max_column):
        ...
        elif isinstance(row[3].value, (int, float)) and suma_columna_3 != 0:
            fila_4 = row[3].value / suma_columna_3 
            distribucion_total +=  row[3].value / suma_columna_3
        else:
            fila_4 = None  
